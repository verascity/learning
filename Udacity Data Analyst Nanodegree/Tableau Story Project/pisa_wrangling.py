...
final_cols = ['Country code 3-character', 'OECD country', 'International Grade', 
              'Gender', 'Attend <ISCED 0>', 'Age at <ISCED 1>', 'Mother<Highest Schooling>', 
...
clean_col_names = ['Country', 'OECD', 'International Grade', 'Gender',
                   'Attended Pre-Primary', 'Age At School Start', 'Mother Highest Schooling',
...
pisa_final = pisa_final[pisa_final['International Grade'] <= 12] # Cleaning up some weird values that don't detract from the analysis

pisa_final = pisa_final[pisa_final['Age At School Start'] <= 13] # Cleaning up some incongruent-seeming ages

pisa_final[pisa_final['Country'] == 'United States'] = pisa_final[pisa_final['Country'] == 'United States'].replace({'Non-OECD':'OECD'}) # Fixing bug

pisa_final.to_csv('pisa_final.csv', chunksize=20000, index=False)